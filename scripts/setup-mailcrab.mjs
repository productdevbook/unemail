  ...
  return new Promise((resolve, reject) => {
    ...
    exec("docker --version", (error, stdout) => {
      if (error) {
        ...
        console.log(
          `${colors.yellow}Please install Docker from https://www.docker.com/get-started${colors.reset}`,
        )
...
function checkPorts() {
  return new Promise((resolve, reject) => {
    console.log(
      `${colors.yellow}Checking if ports ${MAILCRAB_PORT_SMTP} and ${MAILCRAB_PORT_UI} are available...${colors.reset}`,
    )

    const netstat = process.platform === "win32" ? "netstat -ano | findstr" : "lsof -i"
    ...
    exec(`${netstat} :${MAILCRAB_PORT_SMTP}`, (error, stdout) => {
      ...
      exec(`${netstat} :${MAILCRAB_PORT_UI}`, (error, stdout) => {
        ...
        if (smtpInUse || uiInUse) {
          const portsInUse = []
          if (smtpInUse) portsInUse.push(MAILCRAB_PORT_SMTP)
          if (uiInUse) portsInUse.push(MAILCRAB_PORT_UI)

          console.log(
            `${colors.red}❌ Port(s) ${portsInUse.join(", ")} already in use${colors.reset}`,
          )
  ...
  return new Promise((resolve, reject) => {
    ...
    exec(`docker ps --filter ancestor=${DOCKER_IMAGE} --format "{{.ID}}"`, (error, stdout) => {
      ...
      if (runningContainerId) {
        console.log(
          `${colors.yellow}MailCrab is already running with container ID: ${runningContainerId}${colors.reset}`,
        )
        ...
        confirmChoice.question(
          `${colors.yellow}Do you want to stop it and start a new instance? (y/N): ${colors.reset}`,
          (answer) => {
            confirmChoice.close()
            ...
            if (answer.toLowerCase() === "y") {
              exec(`docker stop ${runningContainerId}`, (error) => {
                if (error) {
                  console.log(
                    `${colors.red}❌ Failed to stop MailCrab container: ${error.message}${colors.reset}`,
                  )
                  reject(error)
                  return
                }
        ...
        exec('docker ps -a --filter name=unemail-mailcrab --format "{{.ID}}"', (error, stdout) => {
          ...
          if (stoppedContainerId) {
            console.log(
              `${colors.yellow}Found stopped MailCrab container with ID: ${stoppedContainerId}${colors.reset}`,
            )
            ...
            confirmChoice.question(
              `${colors.yellow}Do you want to (s)tart the existing container, (r)emove it and create a new one, or (c)ancel? (s/r/c): ${colors.reset}`,
              (answer) => {
                confirmChoice.close()

                if (answer.toLowerCase() === "s") {
                  console.log(
                    `${colors.yellow}Starting existing MailCrab container...${colors.reset}`,
                  )

                  exec(`docker start ${stoppedContainerId}`, (error) => {
                    if (error) {
                      console.log(
                        `${colors.red}❌ Failed to start existing MailCrab container: ${error.message}${colors.reset}`,
                      )
                      reject(error)
                      return
                    }
                    ...
                    console.log(
                      `${colors.green}✅ Started existing MailCrab container${colors.reset}`,
                    )
                    resolve({ action: "use-existing" })
                  })
                } else if (answer.toLowerCase() === "r") {
                  console.log(
                    `${colors.yellow}Removing existing MailCrab container...${colors.reset}`,
                  )

                  exec(`docker rm ${stoppedContainerId}`, (error) => {
                    if (error) {
                      console.log(
                        `${colors.red}❌ Failed to remove existing MailCrab container: ${error.message}${colors.reset}`,
                      )
                      reject(error)
                      return
                    }
                    ...
                    console.log(
                      `${colors.green}✅ Removed existing MailCrab container${colors.reset}`,
                    )
                    resolve({ action: "create-new" })
                  })
                } else {
